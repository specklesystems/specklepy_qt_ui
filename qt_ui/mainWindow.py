...
try:
    ...
    from speckle_toolbox.esri.toolboxes.speckle.converter.layers import (
        getLayers,
        getAllProjLayers,
    )
...
class SpeckleGISDialog(QMainWindow):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.streams_add_button.setStyleSheet(
                ...
                + f"background-color: rgb{str(COLOR_HIGHLIGHT)};"
                + f"{COLOR}"
                + " }"
            )
            ...
            self.saveSurveyPoint.setStyleSheet(
                "QPushButton {text-align: right;} QPushButton:hover { "
                + f"{COLOR}"
                + " }"
            )
    ...
    def setupOnFirstLoad(self, plugin):
        try:
            ...
            self.saveLayerSelection.clicked.connect(
                lambda: self.populateLayerDropdown(plugin)
            )
    ...
    def setSendMode(self, plugin):
        try:
            ...
            self.sendModeButton.setStyleSheet(
                "border: 0px;" f"color: rgb{str(SPECKLE_COLOR)};" "padding: 10px;"
            )
            self.sendModeButton.setIcon(QIcon(ICON_SEND_BLUE))
        ...
        try:
            ...
            if plugin.btnAction == 0:  # on send
                if branchStr == "":
                    ...
                    branchStr != ""
                    and self.layerSendModeDropdown.currentIndex() == 1
                    and len(plugin.current_layers) == 0
    ...
    def populateLayerDropdown(self, plugin, bySelection: bool = True):
        ...
        try:
            ...
            from speckle_toolbox.esri.toolboxes.speckle.ui.project_vars import (
            ...
            for stream in plugin.current_streams:
                self.streamList.addItems(
                    [
                        ...
                        else f"{stream[1].name}, {stream[1].id} | {stream[0].stream_url.split('/streams')[0]}"
                    ]
                )
